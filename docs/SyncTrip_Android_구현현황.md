...
| 날짜 | 변경 내용 |
|---|---|
| 2026-06-02 | **로그 정리 3건** — ① 앨범 진단 로그 제거(안드로이드 `AlbumExif`, 백엔드 `[앨범 업로드]` + 미사용 로거). ② 백엔드 `application-dev.yml`의 `org.springframework.web: DEBUG`→`INFO`로 변경(RestTemplate이 Google/Gemini API 키·응답 base64를 통째로 찍던 노출 차단). ③ `findTimedOutVotes` 쿼리에 `b.isDeleted=false` 추가 — 삭제된 밴드가 매분 "투표 타임아웃 자동 마감" 스팸 로그를 유발하던 문제 해결 |
| 2026-06-02 | **공유 앨범 사진 90도 회전 수정** — 업로드 압축 시 JPEG 재인코딩으로 EXIF Orientation 태그가 사라져 세로 사진이 눕던 문제. `extractPhotoData`에서 EXIF를 압축보다 먼저 읽어 `TAG_ORIENTATION`을 얻고, `applyExifOrientation`(Matrix 회전/반전)으로 압축 전에 픽셀을 실제 회전. GPS 복구 검증 완료(도쿄 lat=35.716, lng=139.795 정상 저장) |
...
| 2026-06-02 | **일정 편집 장소 삭제 기능 (➕)** — `ScheduleEditScreen` 슬롯 카드에 휴지통(`DeleteOutline`) `IconButton` 추가. 탭 시 확인 다이얼로그(`AlertDialog`) → `ScheduleViewModel.deleteSlot()` 호출. `SyncTripApiService.deleteScheduleSlot`(`@DELETE schedule/{scheduleId}`) + `ScheduleRepository.deleteSlot` 추가. **백엔드** `DELETE /api/bands/{bandId}/schedule/{scheduleId}` 신규 — `ScheduleController.deleteSchedulePlace` + `ScheduleService.deleteSchedulePlace`(편집 락 검증 → 슬롯 삭제 → 같은 Day 남은 슬롯 임시 주차 후 `assignTimesInOrder` 재계산 → 전원 알림). (`ScheduleScreen.kt`, `ScheduleViewModel.kt`, `SyncTripApiService.kt`, `ScheduleRepository.kt`, `ScheduleController.java`, `ScheduleService.java`) |
| 2026-06-02 | **국내 일정 영업시간 위반 표시 (➕)** — 기존엔 `band.isOverseas()`일 때만 영업시간 위반을 체크해 국내 일정엔 ⚠ 배지가 전혀 안 떴음. 국내 장소도 Google Places에서 `opening_hours`를 받아 캐싱하므로, **백엔드** 영업시간 맵 구성·위반 체크의 `isOverseas` 게이트를 제거(국내·해외 공통, 데이터 있는 장소만 체크). `openingHoursUnverified`(영업미확인)는 해외 정의 유지. 자정 마감(`"00:00"`)은 `LocalTime.MAX`로 보정해 심야 영업 장소 오탐 방지. 적용 위치: `ScheduleService`(generateManual·assignTimesInOrder·recalculateDayTsp·buildOpeningHoursMapFromPlaces), `SimpleTsp.assignTimes`. ⚠ 플래그는 일정 **생성/재계산 시점**에 저장되므로 기존 일정은 재생성·편집 후 반영됨 |
| 2026-06-02 | **알림 아이콘 비행기 → 위치 핀/컬러 로고 (➕)** — 상태바 알림 아이콘(`ic_stat_notification.xml`)을 비행기 실루엣 → 위치 핀(Material place) 단색 실루엣으로 교체(안드로이드 상태바는 알파만 렌더 → 컬러 로고 불가). `SyncTripFirebaseService.showNotification`에 `setLargeIcon`(컬러 런처 아이콘 `ic_launcher1`) 추가 — 알림 펼침 시 우측에 SyncTrip 로고 표시. `AndroidManifest`에 FCM `default_notification_icon` 메타데이터 추가(백그라운드 알림이 흰 사각형으로 깨지지 않도록). (`ic_stat_notification.xml`, `SyncTripFirebaseService.kt`, `AndroidManifest.xml`) |

**마지막 수정:** 2026-06-02 (일정 편집 장소 삭제 · 국내 영업시간 위반 · 알림 아이콘) | **참조 문서:** `SyncTrip_인수인계문서_v6.md`, `SyncTrip_구현현황.md`
