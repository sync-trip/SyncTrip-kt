...
| USR-028 | 밴드 삭제 (방장) | ✅ 구현 | `TripBandHubScreen` 우측 상단 + `BandViewModel.deleteBand()` | 방장만 노출되는 빨간 "방 삭제" TextButton. 삭제 후 홈 이동 |
...
| `CreateTripScreen` | `TripCreationScreens.kt` | ✅ | ✅ | 2단계 플로우. 여행지 검색 + 캘린더 범위 선택 + createBand() 완료 |
| `AiLoadingScreen` | `TripCreationScreens.kt` | ✅ | ✅ | 3초 폴링 + 일정 API 백업 폴링. 완료 시 `popBackStack()` → hub SCHEDULE 탭 |
| `TripBandHubScreen` | `TripLobbyScreen.kt` | ✅ | ✅ | 하단 NavigationBar 4탭(밴드/일정/정산/사진). 상태별 액션 버튼. 방장 전용 "방 삭제" 버튼 |
...
| 밴드 로비 구조 | 단일 화면 | 하단 NavigationBar 4탭 허브 구조 (`TripBandHubScreen`) — 일정/정산을 별도 라우트 대신 탭으로 임베드 |
...
**마지막 수정:** 2026-05-24 | **참조 문서:** `SyncTrip_인수인계문서_v6.md`, `SyncTrip_구현현황.md`
